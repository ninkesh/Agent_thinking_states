...
export function envelopeCardToEntity(card: EnvelopeCard): NormalizedEntity {
  ...
  return {
    ...
    attributes: {
      ...
      ...(card.phoneTel ? { phoneTel: card.phoneTel } : {}),
      ...(card.visualQuery ? { visualQuery: card.visualQuery } : {}),
      ...(card.note ? { note: card.note } : {}),
      ...(card.ratingText ? { ratingText: card.ratingText } : {}),
      ...(card.brand ? { brand: card.brand } : {}),
      ...(card.originalPrice != null ? { originalPrice: card.originalPrice } : {}),
      ...(card.currency ? { currency: card.currency } : {}),
      ...(card.inStock != null ? { inStock: card.inStock } : {}),
      ...(card.categories?.length ? { categories: card.categories } : {}),
      ...(card.gender ? { gender: card.gender } : {}),
      ...(card.vtonEnabled != null ? { vtonEnabled: card.vtonEnabled } : {}),
    },
    raw: card.raw,
  };
}
