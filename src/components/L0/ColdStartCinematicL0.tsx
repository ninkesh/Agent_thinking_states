/**
 * ColdStartCinematicL0 — demo_cold_start only.
 *
 * Identical to CinematicL0 except:
 *  1. Uses coldStartL0Timeline (mascot fully visible before reasoning starts)
 *  2. Accepts onTimelineComplete — fires after beam settles, triggers 10s hold
 *
 * Do not use this component in the main feed, L0 preview, or any other route.
 */
...
// Slower reasoning reveal for cold-start — gives the copy more room to breathe
const REASONING_RESOLVE_MS = 6000;
...
type Props = {
  ...
  /** Fires after the full sequence settles — ColdStartApp starts 10s hold on this */
  onTimelineComplete: () => void;
};

export default function ColdStartCinematicL0({
  item, reasoning, paused = false, ctaFocused, onCTAClick, alignment, showProducts, onTimelineComplete,
}: Props) {
  ...
  // Stable ref so the callback from inside the GSAP closure always sees the latest value
  ...
  const onCtaRevealDone = useCallback(() => {}, []);
  ...
  return (
    <div ref={containerRef} style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {/* MASCOT — center: standalone above reasoning */}
        {alignment === 'center' && (
