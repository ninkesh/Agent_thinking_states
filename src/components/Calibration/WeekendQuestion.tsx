/* ─────────────────────────────────────────────────────────────────────────────
   WeekendQuestion — "What's your kind of weekend?"
   Q4 of setup flow. Multi-select. 4 options. FLIP center-stage acknowledgement.
   ───────────────────────────────────────────────────────────────────────────── */
...
const OPTIONS = [
  { id: 'slow-home',   label: 'Slow at home',      image: '/images/setup/setup_q4_home.jpg',       attrs: { vibes: ['cozy', 'calm'], categories: ['home', 'wellness'] } },
  { id: 'local',       label: 'Local experiences',  image: '/images/setup/setup_q4_local.jpg',      attrs: { vibes: ['local', 'discovery'], categories: ['food', 'entertainment'] } },
  { id: 'night-out',   label: 'A night out',         image: '/images/setup/setup_q4_nightout.jpg',   attrs: { vibes: ['social', 'nightlife'], categories: ['entertainment', 'food'] } },
  { id: 'out-of-town', label: 'Out of town',         image: '/images/setup/setup_q4_outoftown.jpg',  attrs: { vibes: ['adventure', 'exploration'], categories: ['travel'] } },
] as const;
...
function getReplyLines(selected: OptionId[]): [string, string, string] {
  if (selected.length === 0) return ["All good.", "Noted.", "I'll keep the weekend vibe balanced."];
  if (selected.includes('slow-home') && selected.length === 1) return ['Slow at home.', "That's my kind of weekend too.", "Slow, cozy content — well noted."];
  if (selected.includes('out-of-town') && selected.length === 1) return ['Out of town.', "Love the spirit.", "Travel and escape content, dialed up."];
  if (selected.length === 1) return [OPTIONS.find(o => o.id === selected[0])?.label ?? 'That.', 'Noted.', "I'll factor that into how I pick content."];
...
export default function WeekendQuestion({ profile, onNext, onSkip, onBack, initialSelected }: Props) {
  ...
  return (
    <div ref={rootRef} className="fg-screen" data-step="weekend">
      ...
      <div ref={questionRef} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
        ...
        <div className="fg-q-title" style={{ margin: 0, marginBottom: 18 }}>
          {phase !== 'entering' && (
            <CinematicText text="What's your kind of weekend?" playing={qTyping} speed={0.036} duration={0.40} onDone={handleQuestionDone} />
          )}
        </div>
      </div>
      ...
      <div ref={subtitleRef} style={{ opacity: 0, transform: 'translateY(10px)', fontSize: 'clamp(12px,1.1vw,15px)', fontWeight: 400, letterSpacing: '0.01em', color: 'rgba(167,134,229,0.65)', fontFamily: '"Plus Jakarta Sans",system-ui,sans-serif', marginBottom: 'clamp(28px,4.5vh,48px)' }}>
        Helps me understand the pace and vibe you like
      </div>
