/* ─────────────────────────────────────────────────────────────────────────────
   TVContentQuestion — "What's this TV usually playing?"
   Q1 of setup flow. Multi-select. 4 options. FLIP center-stage acknowledgement.
   ───────────────────────────────────────────────────────────────────────────── */
...
const OPTIONS = [
  { id: 'sports',        label: 'Sports & big moments', image: '/images/setup/setup_q1_sports.jpg',  attrs: { categories: ['sports'] } },
  { id: 'movies-series', label: 'Movies & series',      image: '/images/setup/setup_q1_movies.jpg',  attrs: { categories: ['entertainment'] } },
  { id: 'music',         label: 'Music & performances', image: '/images/setup/setup_q1_music.jpg',   attrs: { categories: ['entertainment'], subCategories: ['music'] } },
  { id: 'news',          label: 'News & infotainment',  image: '/images/setup/setup_q1_news.jpg',    attrs: { categories: ['entertainment'], subCategories: ['news'] } },
] as const;
...
const REPLY_LINES: Record<string, [string, string, string]> = {
  sports:          ['Sports & moments.', 'Got it.',       "I'll make sure live moments don't get buried."],
  'movies-series': ['Movies & series.',  'Got it.',       "Stories and series will have a strong presence here."],
  music:           ['Music.',            'Love that.',    "Performances and music will find their way in."],
  news:            ['News.',             'Makes sense.',  "I'll keep you in the loop without the noise."],
  _multi:          ['Great mix.',        'Noted.',        "I'll balance all of those well."],
  _default:        ['All good.',         'Noted.',        "I'll keep things varied and balanced."],
};

function getReplyLines(selected: string[]): [string, string, string] {
  if (selected.length === 0) return REPLY_LINES._default;
  if (selected.length === 1) return REPLY_LINES[selected[0]] ?? REPLY_LINES._default;
  return REPLY_LINES._multi;
}
...
export default function TVContentQuestion({ profile, onNext, onSkip, onBack, initialSelected }: Props) {
  ...
  function handleDone() {
    ...
    const unselectedEls = OPTIONS
      .map((opt, i) => cardElsRef.current[i])
      .filter((el, i) => el != null && !selected.includes(OPTIONS[i].id)) as HTMLDivElement[];

    if (!selectedCards.length) {
      // Skipping — no cards to animate, just show reply
  ...
  return (
    <div ref={rootRef} className="fg-screen" data-step="tv-content">
      ...
      <div ref={questionRef} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
        ...
        <div className="fg-q-title" style={{ margin: 0, marginBottom: 18 }}>
          {phase !== 'entering' && (
            <CinematicText text="What's this TV usually playing?" playing={qTyping} speed={0.036} duration={0.40} onDone={handleQuestionDone} />
          )}
        </div>
      </div>
      ...
      <div ref={subtitleRef} style={{ opacity: 0, transform: 'translateY(10px)', fontSize: 'clamp(12px,1.1vw,15px)', fontWeight: 400, letterSpacing: '0.01em', color: 'rgba(167,134,229,0.65)', fontFamily: '"Plus Jakarta Sans",system-ui,sans-serif', marginBottom: 'clamp(28px,4.5vh,48px)' }}>
        Choose everything that fits — or skip if it varies
      </div>
